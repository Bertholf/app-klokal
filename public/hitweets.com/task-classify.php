...
<html lang="en">
  <head>
	...
	.user { text-align: center; width: 500px; margin: 20px auto; background: #fff; height: 360px;}
...
/* Classify Users */
	if (isset($_REQUEST["action"])) {
		if ($_REQUEST["action"] == "classify") {
			$query = "UPDATE users SET type_id = ". $_REQUEST["type_id"] ." WHERE id = ". $_REQUEST["id"];
			$update = mysql_query($query) or die ('Error updating user');
		}
		if ($_REQUEST["action"] == "location") {
			$query = "UPDATE users SET location_id = 0 WHERE id = ". $_REQUEST["id"];
			$update = mysql_query($query) or die ('Error updating user');
		}
	}

/* Get Users */
	$query = "SELECT id, name, twitter_handle, location, url, image, image_background, image_header, klout_metric_score, text FROM users WHERE location_id = 1 AND type_id = 99 ORDER BY klout_metric_score DESC LIMIT 0, 50";
	...
	if($results) {
		while ($data = mysql_fetch_array($results)) {
			...
			echo "<div class='user'>";
			echo "	<div style=\"height: 350px; margin: 5px; padding: 5px 25px; background: url('". $image_header . "') top center repeat\">";
			echo "		<a href='http://twitter.com/". $twitter_handle . "' target='_blank' class='profile-picture'><img src='". $image ."' /></a>";
			echo "		<div style='background: #fff; '><h2>". $name ."</h2>";
			echo "		<h4>@". $twitter_handle ."</h4>";
			echo "		<p>". $text ."</p>";
			echo "		<span>". $location ."</span>";
			echo "		<span>". $url ."</span>";
			echo "		<p>Klout: ". $klout_metric_score ."</p>";
			echo "			<a href='?action=classify&id=".$id."&type_id=1'>Influencer</a> - ";
			echo "			<a href='?action=classify&id=".$id."&type_id=2'>Brand</a> - ";
			echo "			<a href='?action=classify&id=".$id."&type_id=4'>Celebrity</a> - ";
			echo "			<a href='?action=classify&id=".$id."&type_id=6'>News/Media</a> - ";
			echo "			<a href='?action=classify&id=".$id."&type_id=7'>Government</a> - ";
			echo "			<a href='?action=classify&id=".$id."&type_id=3'>Noise</a>";
			echo "			<a href='?action=location&id=".$id."'><strong>Not Hawaii</strong></a>";
			echo "		</div>";
			echo "	</div>";
			echo "</div>";
