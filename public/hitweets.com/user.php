...
<html lang="en">
  <head>
	...
	.firsttweet { margin: 0px auto; width: 500px; text-align: center;}
...
<div class="container">
	<div class="page-header">
		<div class="d-block d-block-system g-main">
			...
			<?php
			/* MySQL */
			if (isset($_REQUEST["twitter_handle"])) {
				$twitter_handle = $_REQUEST["twitter_handle"];


				$query = "SELECT u.*,
							(SELECT (COUNT(t.id)+1) as rank FROM users t where t.twitter_date_created < u.twitter_date_created)
						  FROM users u WHERE u.twitter_handle = '". $twitter_handle ."'";
				$result = mysql_query($query) or die ('Error retreiving user');
				if($result) {
					while ($data = mysql_fetch_array($result)) {
					?>
						<h1><?php echo $data["name"]; ?></h1>
						<h3>@<?php echo $data["twitter_handle"]; ?></h3>

						<div class="firsttweet"><blockquote class="twitter-tweet"><p>Loading...</p>&mdash; (@<?php echo $data["twitter_handle"]; ?>) <a href="<?php echo $data["twitter_first"]; ?>" data-chrome="transparent" data-datetime="<?php echo $data["twitter_date_created"]; ?>"><?php echo $data["twitter_date_created"]; ?></a></blockquote></div>

						<h3>You were the #<?php echo $data["rank"]; ?> Twitter user in Hawaii</h3>
